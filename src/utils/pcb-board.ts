...
export function pcbBody(
  ...
  textColor = '#e8f5ec',
  strokeColor = '#0c3a1e',
  // Board-body decorations (connectors, ICs, buttons, LEDs) that should
  // render between the board rect and its silkscreened title - so the
  // title text stays on top (SVG has no z-index; later-drawn wins) even
  // when a decoration's bounding box happens to overlap the title.
